...
- AI-native specialists raised billions in debt and equity to finance GPU purchases at premium prices
...
This phase demonstrated that GPU rental markets can operate in **allocation mode** rather than **price-clearing mode** for extended periods.

### Phase 6: Fragmentation and abstraction (2024–present)

Countervailing forces emerged simultaneously:

- **Custom silicon displacement:** Google TPUs, Amazon Trainium, Microsoft Maia, and startup accelerators captured inference and some training niches
- **Model efficiency gains:** Quantization, distillation, mixture-of-experts sparsity, and smaller capable models reduced FLOPs per capability unit
...
Across every phase, one pattern persists: **headline prices lie**. Organizations measure $/GPU-hour; they rarely measure effective $/completed-training-run, $/successful-fine-tune, or $/inference-request after accounting for interruptions, egress, orchestration overhead, and engineer intervention. The historical arc is not progress toward a unified spot market but progress toward **more sophisticated segmentation**—and more sophisticated ways of extracting scarcity rent from those who cannot internalize hardware.
...
GPU rental markets behave well under steady-state competitive assumptions and badly under the conditions that actually dominate headlines.

### Edge case 1: Egress and storage bill shock

A training run priced at $2/GPU-hour for 500 GPU-hours ($1,000 compute) may generate $5,000–$50,000 in egress charges when checkpoints, datasets, and artifacts cross cloud boundaries. Renters who optimize compute hourly rate while ignoring data gravity systematically mis-budget.

### Edge case 2: Correlated spot interruption during demand spikes

Spot capacity is statistically cheap when interruption is uncorrelated. During industry-wide demand events—major model releases, conference deadlines, funding announcement cycles—spot prices surge simultaneously across providers and interruption rates spike in correlated fashion, destroying the diversification assumption that makes spot attractive.
