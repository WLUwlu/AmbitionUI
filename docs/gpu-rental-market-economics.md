...
GPU rental markets are not a single exchange where homogeneous units clear at one price. They are a **stack of partially overlapping markets** that rent programmable parallel accelerators under incompatible economic contracts. A burst job on a consumer RTX 4090 listed on a peer marketplace, a six-month reserved block of A100s on a hyperscaler, and a three-year dedicated H100 cluster with InfiniBand under a specialist AI cloud differ in price discovery, risk allocation, enforceability of service levels, and—critically—in what fraction of total workload cost is captured by the GPU-hour line item alone.
...
Effective cost per GPU-hour ≈ (Hardware capex / useful life hours)
                            + Power and cooling
                            + Facility amortization (rack, network, staff)
                            + Software and orchestration overhead
                            + Risk premia (eviction, churn, bad debt)
```

**Hardware capex** dominates frontier generations. An H100-class accelerator may cost tens of thousands of dollars per unit; useful competitive life depends on when the next generation (Blackwell and successors) makes prior silicon economically obsolete for frontier training—even if inference workloads remain viable for years.

**Power and cooling** scale nonlinearly with rack density. A 700W GPU in a 120kW rack imposes liquid or advanced air cooling requirements. At $0.08–$0.15/kWh industrial rates, power can exceed thirty percent of marginal cost for dense deployments; at $0.25+ retail or volatile European tariffs, it can dominate.

**Facility amortization** includes data center buildout, network switches, InfiniBand or NVLink fabric, and operational staff. Cluster training economics require fabric investment that single-card rental listings ignore entirely—a source of persistent price dispersion between "a GPU" and "a trainable cluster."

**Utilization** is the multiplier that converts fixed costs into viable unit economics. A host at twenty percent utilization needs five times the hourly revenue of one at one hundred percent to recover the same capex—explaining why spot discounts exist and why hoarding during shortage is rational for providers who can sell reserved capacity instead.

### Demand-side segmentation

| Segment | Price sensitivity | Utilization pattern | Contract preference |
|---------|-------------------|---------------------|---------------------|
| Frontier labs | Low during shortage | Sustained cluster jobs | Multi-year, prepay |
| Enterprise ML | Moderate | Mixed training + inference | Reserved + on-demand |
| Startups / researchers | High | Bursty, experimental | Spot, marketplace |
| Inference SaaS | Moderate–high | Steady with spikes | Autoscale, regional |
| Hobbyists / students | Very high | Episodic | Peer marketplace |

Demand elasticity varies by segment and macro cycle. During AI hype peaks, **budget constraints relax** and reservation queues lengthen. During consolidation or model-efficiency breakthroughs, demand can fall faster than depreciation—creating oversupply and price wars among hosts with stranded inventory.

### Pricing regimes and arbitrage

Four canonical pricing regimes coexist:
...
Peer marketplaces exhibit classic platform economics: **cross-side network effects** (more hosts attract more renters and vice versa), **quality uncertainty** (renters cannot fully observe host reliability pre-purchase), and **multi-homing** (renters compare across Vast.ai, RunPod, and hyperscaler spot simultaneously). Trust mechanisms—verified hosts, uptime scores, escrow—substitute for enterprise SLAs at lower cost but never fully eliminate variance.

Hyperscalers internalize trust through brand and legal recourse but face **organizational inertia**: GPU product teams compete with internal first-party AI product teams for allocation, creating internal transfer pricing that external observers cannot see.
...
Managed model APIs charge per token or per request, bundling compute, model weights, and operational expertise. For application developers without ML infrastructure teams, **effective $/token** may beat self-hosted inference even when raw GPU rental appears cheaper—because the comparison omits MLOps labor, model serving engineering, and autoscaling complexity. This trade-off increasingly segments the market: infrastructure renters versus API consumers.
...
**Multi-tenant interference.** Shared hosts without strict isolation may suffer memory bandwidth contention or PCIe saturation when neighbors run dissimilar workloads. Benchmarked TFLOPs on empty hardware misrepresent **realized throughput** under multi-tenant load.

**Insurance and liability gaps.** Peer marketplaces rarely offer indemnification for data loss, model weight leakage, or regulatory breach. Enterprise procurement treats this as a hard stop regardless of hourly savings—a boundary that keeps residual markets residual.
...
**Limitation 8 — Environmental externalities omitted.** Carbon intensity of regional grids and cooling water use affect long-run regulatory and reputational costs for hosts; these are rarely priced into hourly rates today but may become material under expanded disclosure regimes.
