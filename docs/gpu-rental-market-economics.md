...
GPU rental markets are routinely collapsed into a single headline—“GPU cloud pricing”—that obscures more than it reveals. In practice, at least four distinct economic regimes coexist under that umbrella: hyperscaler metered instances with enterprise procurement overlays; specialist AI infrastructure firms selling cluster-scale capacity on contract; peer-to-peer marketplaces matching individual hosts to renters; and internal transfer markets where large labs allocate silicon across teams without external price discovery. A researcher renting a single RTX 4090 for a weekend experiment, a startup reserving thirty-two A100s for six weeks, and a Fortune 500 firm signing a multi-year H100 block lease are all “renting GPUs,” yet they inhabit different pricing regimes, bear different failure risks, and optimize against different constraints.

This analysis treats GPU rental as a **capital-intensive leasing market for rapidly depreciating accelerators**, embedded inside broader platform ecosystems and subject to periodic rationing shocks. The hourly rate displayed on a pricing page is a compressed summary of at least seven independent economic forces: hardware amortization and residual value; site-level energy and cooling economics; network and storage attachment; software stack compatibility and certification; trust, compliance, and security guarantees; orchestration and idle-time overhead; and the option value of guaranteed allocation during shortage cycles. Any procurement decision based solely on the headline $/GPU-hour figure is structurally incomplete.

**Scope boundaries:** The focus is general-purpose GPU rental for machine learning training, fine-tuning, and batch inference. Cryptocurrency mining, FPGA rental, and proprietary AI accelerators (Google TPU, Amazon Trainium, Microsoft Maia, Groq LPU) appear only where they materially affect GPU supply, demand, or pricing. Published list prices are illustrative; transactional prices during rationing periods can diverge by multiples from advertised rates.
...
**Premise 1 — Differentiated commodity:** At the silicon layer, an H100 SXM module running a standard CUDA stack approaches fungibility. At the service layer—SLA tier, data residency, fabric topology, support response time—products diverge enough to sustain 3–10× price spreads for nominally identical hardware.

**Premise 2 — Shortage suspends textbook markets:** During allocation-constrained periods (roughly 2023–2025 for H100-class hardware, with echoes into 2026), price ceases to clear supply and demand in the competitive sense. Queue priority, relationship capital, prepayment, and geographic eligibility replace marginal-cost pricing. Analyses trained on competitive-market assumptions systematically underpredict realized prices during these windows.
