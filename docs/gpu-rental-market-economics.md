...
GPU rental markets are routinely described with commodity metaphors: an hourly price for a known chip, fungible across vendors, comparable like crude oil or container shipping rates. That metaphor is useful for procurement spreadsheets and dangerously incomplete for strategic decisions. GPU rental is not one market but a **layered stack of partially overlapping markets** that share silicon branding while differing in price discovery, contractual risk allocation, and the implicit services compressed into the headline rate.

A hobbyist renting a single RTX 4090 on a peer-to-peer host, a seed-stage startup reserving eight H100 nodes for six weeks on a specialist provider, and a frontier lab signing a multi-year dedicated cluster contract with CoreWeave are all "renting GPUs." They inhabit different economic regimes, face different failure modes, and should not be analyzed with a single demand curve.

This analysis treats GPU rental as a **capital-intensive leasing market for rapidly depreciating accelerators**, embedded inside broader platform ecosystems: hyperscale cloud, AI-native specialists, colocation bare metal, and decentralized host networks. The hourly sticker price is a lossy compression of at least seven independent cost and value drivers:

1. Hardware amortization and residual-value forecasting  
2. Site-level energy, cooling, and power-density economics  
3. Network, storage attachment, and data-movement charges  
4. Software stack compatibility, certification, and driver curation  
5. Trust, security, compliance, and SLA guarantees  
6. Orchestration quality and developer experience  
...
Decentralized marketplaces offer price and variety advantages but impose **trust and variance costs**: host reliability varies from data-center-grade to residential broadband with gaming cards; security models assume container isolation with GPU memory inspection attacks remaining a concern without confidential computing; geographic distribution creates latency and data-residency complexity; support is community-mediated rather than contractually guaranteed.
...
Hyperscaler GPU instances advertise competitive compute rates but charge aggressively for data egress and high-IOPS storage. A training run generating terabytes of checkpoint artifacts can incur storage and transfer charges exceeding compute costs—a failure mode invisible in $/GPU-hour procurement spreadsheets. Effective cost analysis must price **total workload economics**, not socket occupancy alone.

### Edge case 2: Correlated spot eviction during demand spikes

Spot instances assume independent interruption probability. In practice, providers reclaim spot capacity en masse when reserved customers demand allocation—creating **correlated eviction events** during exactly the periods when spot was cheapest. Teams running large hyperparameter sweeps without checkpoint granularity discover that 50% spot savings can convert to 100% project delay.
