...
Early platforms subsidized one side (low host fees, renter credits) to bootstrap liquidity—a classic platform investment with uncertain payback.

### Interconnect as a separate good

Multi-GPU training treats **network fabric** as a distinct economic input. An 8-GPU H100 node with NVLink is not substitutable for 8 discrete PCIe-attached GPUs. Providers price **topology** (single node, rack, pod, region) because scaling efficiency depends on all-reduce bandwidth. This creates **natural monopoly pockets** within otherwise competitive markets: only certain providers can offer 1,024-GPU InfiniBand fabrics at scale.

### Inventory and utilization dynamics

Unlike SaaS, GPU rental inventory is **finite and depreciating**. Providers optimize:

- **Fill rate** — minimize idle GPUs
- **Yield management** — segment spot vs reserved
- **Preemption hierarchy** — spot evicted for reserved

Airline seat pricing and hotel yield management are closer analogies than software licensing. Empty GPU-hours are **perishable inventory**—once the hour passes, revenue is lost forever.

### Financing and capital structure

Frontier GPU deployment requires **asset-heavy financing**: debt secured against hardware, sale-leaseback structures, vendor financing from NVIDIA/OEMs. Interest rates and allocation access determine who can scale supply during shortage. A host with NVIDIA allocation but no capital still cannot compete; a host with capital but no allocation holds empty powered racks—**stranded capital**.
...
### Trade-off 8: Centralization vs fragmentation of price discovery

A fragmented marketplace layer (dozens of hosts, volatile spot) produces **noisy signals** about true marginal cost. Hyperscaler list prices act as **focal points** even when economically unjustified—anchoring enterprise procurement. Renters may overpay for AWS/GCP/Azure because **budget predictability** and **vendor approval** outweigh marginal savings from specialists.
...
### Edge case 11: Cold-start latency and bursty workloads

Inference renters optimizing for autoscaling may pay for **idle warm GPUs** to avoid cold-start penalties. Quoted $/GPU-hour understates cost when utilization is low but availability must be instant—akin to **insurance against latency**.

### Edge case 12: Egress fee traps

A training job completing on cheap compute but exporting terabytes of checkpoints or datasets to another region can incur **egress charges** exceeding compute cost. Total workload economics invert; naive $/GPU-hour comparison misleads procurement.

### Failure mode synthesis

Markets fail visibly when: (a) shortage replaces price with queue; (b) correlated spot interruptions; (c) hidden egress/storage charges dominate; (d) hardware obsolescence outpaces amortization schedules; (e) trust breakdown in P2P layers; (f) power unhedged against volatile energy markets.
...
**Limitation 7: Inference API abstraction.** Managed model APIs (OpenAI, Anthropic, etc.) collapse rental economics into per-token pricing, hiding underlying GPU utilization. This analysis treats rental markets explicitly; API markets are **opaque derivatives** of the same supply chain.
