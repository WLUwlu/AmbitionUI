...
Effective cost per GPU-hour ≈ (Hardware capex / useful life hours)
                            ...
                            + Risk premia (utilization uncertainty, obsolescence)
```

**Hardware capex** dominates the fixed-cost stack. Frontier H100-class accelerators carry list prices in the tens of thousands of dollars per unit; useful competitive life may be two to three years before a successor generation offers sufficient performance-per-dollar improvement to strand prior inventory. Amortization assumes a utilization rate: at fifty percent fleet utilization, effective hourly depreciation doubles versus one hundred percent—a lever that separates profitable hosts from distressed ones.

**Power and cooling** scale nonlinearly with rack density. A 10 kW GPU rack in a region at $0.04/kWh costs roughly $0.40/hour in energy alone before PUE overhead; at $0.15/kWh, the same rack costs $1.50/hour. Locational arbitrage is therefore not cosmetic—it can exceed margin on compute for undifferentiated hosts.

**Facility and network** costs include colocation fees, InfiniBand switches (often six figures per spine), cross-AZ egress, and staffing for 24/7 operations. Specialist AI clouds amortize fabric across cluster contracts; peer hosts often externalize network quality, accepting consumer uplinks.

**Utilization** is the master variable. Hyperscalers cross-subsidize GPU fleets with CPU, storage, and SaaS attach; specialist clouds live or die on GPU utilization alone. Peer marketplaces aggregate fragmented idle capacity whose opportunity cost is near zero for hobbyist hosts but nonzero for commercial operators.

### Demand-side segmentation

Demand is not monolithic. Segments include:

1. **Frontier labs** — Multi-thousand-GPU training runs; price-inelastic on availability; contract duration measured in quarters or years.
2. **Enterprise fine-tuning** — Hundreds of GPUs intermittently; sensitive to compliance and support; willing to pay hyperscaler premia.
3. **Startups and researchers** — Burst-shaped, spot-tolerant, price-elastic; primary peer marketplace constituency.
4. **Inference operators** — Latency-sensitive; prefer regional placement and autoscaling; increasingly served by managed APIs.
5. **Non-ML workloads** — Rendering, simulation, video transcoding; compete for the same hardware during off-peak.

Each segment exhibits different elasticity with respect to price, reliability, and contract length. Aggregating "GPU demand" without segment labels produces misleading forecasts.
