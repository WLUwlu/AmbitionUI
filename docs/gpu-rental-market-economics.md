...
**Premise 4:** Total cost of ownership for a workload spans compute, storage, egress, checkpoint I/O, orchestration overhead, and engineer time—$/GPU-hour is necessary but insufficient for rational procurement.
...
### Structural analogy: aviation leasing and bulk shipping

GPU rental increasingly resembles ** aircraft spare engine leasing** and **container shipping** more than SaaS:

- Long-lived capital assets with technology obsolescence risk
- Boom-bust cycles driven by exogenous demand shocks
- Forward contracts and asset-backed financing
- Utilization as the central profitability lever

This analogy helps explain why specialist providers raise debt against GPU collateral and why renters during shortages accept take-or-pay structures that would be absurd in pure software markets.
...
Early platforms subsidized one side (low host fees, renter credits) until liquidity crossed a critical threshold—classic platform economics.

### Pricing mechanisms in practice

**On-demand list pricing:** Fixed hourly rate; simple; often 2–5× spot during normal times.

**Reserved / committed use:** Prepay or minimum spend for 1–3 year terms; provider gains utilization certainty; renter gains 30–60% discount.

**Spot / interruptible:** Auction or surplus pricing; price floats with utilization; eviction risk explicit.

**Marketplace dynamic pricing:** Hosts set minimum bids; renters search/sort; quality variance high.

**Cluster contracts:** All-in pricing for racks (GPU + fabric + storage tier); negotiated; opaque; dominates frontier training.

### Financing and capital structure

Specialist GPU clouds are **infrastructure financiers** disguised as SaaS:

- Asset-backed lending against GPU inventory
- Prepaid multi-year contracts used as collateral
- JV structures with energy companies (stranded gas → on-site power → GPU hosting)

When interest rates rise, the **carry cost** of hoarded H100 inventory increases, pressuring providers to monetize idle fleet at spot—even if spot depresses long-term contract anchors.

### Interconnect as economic bottleneck

Single-GPU hourly pricing misleads for distributed training. All-reduce communication across nodes makes **effective throughput** a function of:

- NVLink within node
- InfiniBand or RoCE across nodes
- Network topology (fat-tree vs torus)

A "cheap" GPU-hour on poorly networked hosts can increase **wall-clock time superlinearly**, raising total project cost above premium fabric-inclusive providers.

---

## Section IV — Trade-offs and Strategic Decisions
...
OpenAI, Anthropic, and similar APIs bundle inference into token pricing. For many applications, **effective $/inference** under managed APIs beats self-hosted GPU rental when accounting for MLOps labor, autoscaling complexity, and idle capacity. Raw rental wins at scale, predictable traffic, and custom model requirements.
...
MIG and time-slicing schedulers can **oversubscribe** memory or compute, promising fractional GPUs to multiple tenants. Under burst load, performance collapses—a hidden form of noisy-neighbor economics not reflected in list price.

### Edge case 12: Cooling and climate boundary conditions

Air-cooled consumer hosts in hot climates hit thermal throttle during sustained training loads, reducing effective FLOPs per billed hour. Data-center-grade liquid cooling shifts the feasible density frontier; comparing hourly rates across cooling tiers is apples-to-oranges.

### Failure mode synthesis

Markets fail visibly when: (a) shortage replaces price with queue; (b) correlated spot interruptions; (c) hidden egress/storage charges dominate; (d) hardware obsolescence outpaces amortization schedules; (e) trust breakdown in P2P layers; (f) oversubscribed fractional instances create latent quality collapse.
...
**Limitation 7: Survivorship in marketplace data.** Reported average marketplace prices overweight hosts who stay online; churned failed hosts disappear from aggregates, biasing downward variance estimates.
