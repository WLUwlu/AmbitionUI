...
**Premise 4:** Effective procurement requires pricing **workloads**, not chips. Storage ingress/egress, checkpoint frequency, network latency, and human operator time frequently dominate raw $/GPU-hour for small and mid-size teams.
...
During this phase, the rental market was **thin**. Demand came from scientific computing and early deep learning. Supply was concentrated in three hyperscalers. Price discovery was opaque list pricing with reserved-instance discounts. The economic function of rental was **risk transfer**: convert capex and facility ops into opex for teams uncertain about workload persistence.
...
This phase teaches a permanent lesson: **GPU rental competes with any workload that monetizes flops/watt**, not only other ML jobs. Providers who assumed ML-only demand models were blindsided by cross-market elasticity.
...
The shortage era resembled **semiconductor allocation during automotive crises**: list price mattered less than queue position and relationship capital.
...
The market is bifurcating: **frontier training clusters** (scarce, contract-heavy) vs **inference fleet** (more competitive, software-scheduled). Managed inference APIs (OpenAI, Anthropic, Together) further abstract the rental layer for many buyers, converting GPU economics into token pricing.
...
### Pricing dynamics: spot, reserved, and contract markets

Three pricing regimes coexist:

1. **Spot / on-demand:** Clears marginal supply and demand hourly. Volatile; reflects utilization gaps and shortage psychology.
2. **Reserved / committed:** Renters prepay or commit capacity for 1–3 years; providers offer 30–60% discounts in exchange for utilization guarantees.
3. **Bespoke cluster contracts:** Multi-year, allocation-linked deals with delivery schedules—closer to **project finance** than cloud billing.

During equilibrium, spot prices anchor expectations. During shortage, **contract markets lead** and spot becomes thin or illiquid.

### Interconnect and cluster economics

Single-GPU pricing is misleading for modern LLM training. A training job's economic unit is often **GPU-node-hour** or **GPU-rack-hour** with specified NVLink/InfiniBand topology. Providers bundle:

- Fat-tree or rail-optimized networks
- Shared filesystem throughput (Weka, Lustre, etc.)
- Job schedulers (Slurm, Kubernetes with GPU operators)

A renter comparing $2/GPU-hour on a marketplace vs $4/GPU-hour on a specialist cloud may ignore that the cheaper option lacks **all-reduce bandwidth** sufficient for 512-GPU training—making effective economics inverted.

### Competitive structure and market power

Market concentration operates at multiple layers:

- **NVIDIA** controls supply of frontier accelerators and sets reference architecture
- **Hyperscalers** (AWS, Azure, GCP) control enterprise distribution and bundle GPU with broader cloud contracts
- **Specialist AI clouds** compete on allocation access, financing speed, and ML-specific UX
- **Decentralized marketplaces** compete on price at the long tail

No single layer is perfectly competitive. Renters often face **oligopoly pricing** at the frontier and **monopolistic competition** at the long tail.
...
Buying tokens from an inference API eliminates scheduling, scaling, and hardware selection—but embeds provider margin and reduces transparency. Economically rational when:

- Workload is inference-only with variable load
- Team lacks MLOps capacity
- Latency SLA is standardized

Raw rental wins for training, custom architectures, and data-sovereignty requirements.
...
MIG and time-slicing allow providers to sell more than 100% of a GPU's physical capacity. Under contention, **effective performance collapses**—analogous to CPU oversubscription in early VPS markets.

### Edge case 12: Cold-start latency for serverless GPU

Serverless GPU products bill only for inference time but impose **cold-start penalties** (model load to VRAM). Workloads with sparse requests may pay low hourly rates while experiencing **terrible p99 latency**—hidden cost in SLA terms, not dollars per hour.

### Failure mode synthesis

Markets fail visibly when: (a) shortage replaces price with queue; (b) correlated spot interruptions; (c) hidden egress/storage charges dominate; (d) hardware obsolescence outpaces amortization schedules; (e) trust breakdown in P2P layers; (f) oversubscription without transparent contention signaling.
...
**Limitation 7: Survivorship bias in marketplace data.** Published spot prices reflect active listings, not failed hosts who exited after unprofitable utilization. True supply-side economics may be worse than visible market depth suggests.
