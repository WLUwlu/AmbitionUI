...
GPU rental markets occupy an uneasy middle ground between commodity hardware leasing and platform-mediated cloud services. A PhD student renting a consumer RTX 4090 for $0.40 per hour on a peer-to-peer marketplace, a Series B startup reserving a 128-node H100 cluster for six weeks, and a Fortune 500 bank purchasing a three-year committed-use contract on Azure ND-series instances are all, in a loose sense, "renting GPUs." Yet they inhabit different pricing regimes, bear different failure modes, and optimize against different implicit subsidies. Treating these experiences as a single market produces procurement advice that is simultaneously too cautious for some participants and recklessly optimistic for others.
