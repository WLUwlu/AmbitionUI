...
Hosts with underutilized local hardware gained yield on sunk capital; renters accessed rates often materially below hyperscaler list during non-shortage periods—at the cost of reliability variance, support fragmentation, security ambiguity, and driver drift. This layer introduced **price discovery in the long tail** that hyperscaler administrative pricing could not replicate, while simultaneously exposing renters to quality heterogeneity that enterprise procurement processes struggle to absorb.

### Era 5: Transformer scarcity and institutional bifurcation (2022–present)

The ChatGPT inflection and subsequent frontier model race transformed GPU rental from a specialized infrastructure line item into **strategic national and corporate priority**. H100-class allocation became the binding constraint for serious training programs. Several structural shifts crystallized:

- **AI-native infrastructure firms** (CoreWeave, Lambda, and peers) scaled as dedicated GPU landlords with NVIDIA channel relationships and AI-specific facility design, competing with hyperscalers on depth rather than breadth.
- **Multi-year prepay contracts** emerged as capex financing instruments, transferring technology-obsolescence risk toward renters willing to guarantee demand visibility.
- **Sovereign AI initiatives** (national compute programs, regional cloud mandates) segmented supply along political and compliance boundaries.
- **Managed inference APIs** (OpenAI, Anthropic, and hyperscaler model services) began absorbing GPU economics into opaque per-token pricing, decoupling application developers from hardware procurement entirely.

The Blackwell generation (B200 and successors) extends this era: higher power density, liquid cooling requirements, and even longer lead times for cluster-scale deployment reinforce that **frontier rental is increasingly a capital-markets and geopolitics story**, not merely a cloud console transaction.
...
A hobbyist listing idle gaming GPUs on a marketplace has near-zero incremental cost for an additional billed hour—the sunk hardware is already paid for. This creates a **price floor below commercial break-even** during glut, forcing professional hosts to compete with subsidized supply that does not amortize facility, staff, or replacement capex. The marketplace mean price can fall below sustainable commercial hosting economics without signaling market equilibrium.

### Edge case 2: Correlated spot evictions

Spot pricing assumes independent eviction risk; in practice, reclamation events correlate with macro demand spikes, regional capacity crunches, and provider maintenance windows. Teams running large spot fleets without checkpoint discipline experience **catastrophic correlated failure**—all nodes reclaimed simultaneously—destroying days of training progress. Effective cost explodes even when average $/hour looked attractive.

### Edge case 3: Egress and storage dominance

A renter optimizing compute hourly rate may find that **egress fees and high-performance storage** dominate total spend for data-heavy training pipelines. The GPU-hour is the visible line item; the hidden bill is petabyte-scale dataset movement and checkpoint I/O. Providers with bundled storage and network pricing obscure cross-subsidy; those with à la carte pricing surprise renters who modeled only compute.

### Edge case 4: Driver and firmware drift

Marketplace hosts running heterogeneous driver versions produce **non-reproducible training behavior**—numerical divergence, performance cliffs, or outright kernel failures. Economic cost manifests as engineer debugging time, not invoice line items. Enterprise SLAs exist partly to cap this heterogeneity tax.

### Edge case 5: Multi-tenant security and compliance failure

Shared infrastructure on marketplace tiers may violate enterprise security requirements (SOC 2, ISO 27001, sector-specific mandates) regardless of price attractiveness. A renter who procures cheap GPU-hours and later fails audit faces **retroactive compliance cost** exceeding any compute savings. Compliance is a hard constraint, not a preference curve.

### Edge case 6: Fabric partition in training clusters

Large-scale synchronous training requires low-latency, high-bandwidth interconnect. A "cluster" sold as N GPUs without guaranteed NVLink/InfiniBand topology may be **economically useless for frontier training** despite appearing competitively priced per GPU. Mis-specified topology is a classic adverse-selection problem: uninformed renters discover the gap only after contract signature.

### Edge case 7: Phantom capacity

Marketplace listings may advertise GPUs that are intermittently unavailable, oversubscribed, or mis-specified (consumer vs data-center, VRAM size, PCIe generation). **Reported capacity exceeds deliverable capacity**, a form of inventory fraud that degrades market trust and pushes risk-averse renters back toward hyperscaler premiums.
