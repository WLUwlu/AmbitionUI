...
GPU rental is often discussed as if it were a single, transparent commodity market—like crude oil or container shipping—when it is better understood as a **layered stack of partially overlapping markets** that happen to share silicon branding. A hobbyist renting a single RTX 4090 on a peer-to-peer host, a mid-size AI lab reserving eight H100 nodes for six weeks on a specialist provider, and a Fortune 500 firm signing a multi-year dedicated cluster contract with CoreWeave or Lambda are all "renting GPUs," yet they participate in different price-discovery regimes, bear different failure risks, and optimize against different implicit subsidies.

This analysis treats GPU rental as a **capital-intensive leasing market for rapidly depreciating accelerators**, embedded inside broader platform ecosystems (hyperscale cloud, AI-native specialists, decentralized host networks). The hourly sticker price is a compressed signal that hides at least seven independent cost and value drivers: hardware amortization and residual value, site-level energy and cooling economics, network and storage attachment, software stack compatibility and certification, trust and compliance guarantees, orchestration and developer experience, and the option value of scarce allocation during shortage cycles.
