...
Platform teams emerged as internal service providers with their own rotations, creating **layered on-call**: product teams, platform teams, security, data, networking—each with seams where incidents stall. Internal platform economics reproduced external vendor dynamics: downstream teams felt powerless; upstream teams felt unappreciated.

### Regulatory attention and labor mobilization (2020s)

European right-to-disconnect legislation, growing union activity in tech (particularly among game developers, warehouse-adjacent tech workers, and some engineering cohorts), and public discourse about burnout forced partial recognition that **availability is labor**. Some jurisdictions treat compensable on-call differently depending on whether the worker must remain on-premises or merely reachable—a legal distinction that sociology shows collapsing in practice when Slack pings arrive at dinner tables regardless of formal classification.
