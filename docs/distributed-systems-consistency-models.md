...
The vocabulary of distributed consistency did not arrive fully formed from a single laboratory. It accreted over six decades as engineers discovered, again and again, that replication—deployed to improve throughput, survivability, and geographic proximity—quietly annihilates the comforting fiction that a cluster behaves like one computer. Each named guarantee in the modern taxonomy is, in part, institutional memory: a contract written after an outage, a lost write, or a double-charge that someone decided must never recur without at least a label to assign blame.

The first fractures appeared inside single cabinets, not across continents. When multiple processors began sharing memory in the 1960s and 1970s, programmers inherited a uniprocessor mental model: memory is passive, and a read returns the latest write. Cache hierarchies violated that assumption at hardware speed. Coherence protocols such as MESI and its descendants restored a usable local fiction. Networked systems face a strictly harder problem. There is no shared bus. Message delay is unbounded relative to local computation. Nodes fail independently. Any guarantee that pretends otherwise must be engineered, priced, and defended in incident reviews.

Leslie Lamport's 1978 work on logical clocks and the happens-before relation supplied distributed systems with durable vocabulary for ordering events without trusting wall clocks. Causality follows communication and dependency, not synchronized time-of-day readings. Lamport timestamps, vector clocks, version vectors, and the causal consistency arguments of the 1990s descend from this insight. The lesson remains painfully current: when NTP drifts, VMs freeze, or operators manually adjust clocks, logical order survives where timestamp order collapses.

The 1980s distributed file systems—NFS, AFS, Sprite—shipped behavioral contracts long before most application developers knew to ask for them. Close-to-open semantics, lease-based invalidation, and implicit session stickiness were operational answers to a question rarely posed in product requirements: after a write completes on one node, what may a reader legally observe elsewhere? Operators learned these semantics through corrupted files and mysterious staleness, not through formal specifications.

Commercial databases in the 1980s and 1990s anchored correctness in ACID transactions and isolation levels. SQL-92 codified phenomena—dirty reads, non-repeatable reads, phantoms—and elevated serializable execution as the relational gold standard. Weaker isolation traded anomaly prevention for throughput on contended OLTP workloads, a bargain that seemed acceptable until multi-site replication fractured the contract. A serializable primary in one region could coexist with read replicas lagging seconds behind. Application developers invented read-from-primary-after-write patterns, sticky sessions, and client-side tokens years before those behaviors acquired names like read-your-writes or session consistency.
...
Transactional consistency addresses multi-key atomicity: transactions appear equivalent to some serial execution when isolation is serializable, or to weaker schedules when isolation is relaxed. Snapshot isolation, read committed, and repeatable read each permit specific anomaly classes. Distributed transactions across shards introduce additional failure modes—prepared-but-not-committed states, coordinator crashes, and heuristic commits—that single-node isolation literature does not fully capture.

Consensus is the mechanism; consistency is the contract. Raft, Paxos, and Multi-Paxos elect leaders and order log entries. What clients observe depends on how reads are routed—leader-only versus quorum reads—and whether stale followers may serve traffic. A system can run Raft internally yet expose eventually consistent reads if the API permits replica serving without version checks.

Scope matters as much as strength. A guarantee may hold per key, per shard, per session, per region, or globally. Misunderstanding scope produces the classic failure mode: the database is "strong" while the search index, cache, and CDN are eventual, and no one documented the composition rules. Consistency is therefore not a single knob but a family of contracts whose meaning is incomplete without specifying the observer, the object set, the failure assumptions, and the time horizon.
