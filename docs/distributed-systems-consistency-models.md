...
Amazon's Dynamo paper (2007) operationalized **eventual consistency** with **vector clocks**, **quorum reads/writes**, and **conflict resolution at read time (read repair)**. Google Bigtable (2006) and later Spanner (2012) pushed the opposite direction: **externally consistent** distributed transactions at planetary scale, using TrueTime (GPS/atomic-clock-assisted bounded clock uncertainty) to implement commit-wait for external consistency—a stronger guarantee than many practitioners knew they needed.
...
Most consistency literature assumes **crash-stop** faults: nodes halt but do not lie. **Byzantine** faults (arbitrary behavior, including malicious lies) require different protocols (PBFT, blockchain consensus) with higher overhead. A system "linearizable under crash faults" may not be safe under Byzantine adversaries. Security and consistency intersect when replicas can be compromised or when clients must not trust unauthenticated reads.
...
|-------|-------------------|------------------------|---------------------------|-------------------|
| Linearizable | Acts like one copy, now | Highest coordination cost; sensitive to WAN RTT | Consensus per write or lease + fencing | Locks, leader election, inventory holds, metadata coordination |
| Serializable transactions | Multi-key transactions appear serial | High; conflict detection/retry storms possible | 2PL, OCC, Calvin-style ordering, Spanner-like timestamps | Financial transfers, relational invariants across rows |
| Causal | Cause precedes effect globally | Moderate; metadata (version vectors) on paths | Vector clocks, dependency tracking | Social feeds, commenting threads, collaborative docs (partial) |
| Read-your-writes / session | "My edits show up for me" | Low incremental cost with sticky routing | Session tokens, client-side versioning | User profile updates, shopping carts |
| Eventual | Replicas converge later | Lowest write latency; high read flexibility | Async replication, CRDT merge, anti-entropy | DNS, analytics counters, likes, passive caches |
| CRDT-strong (per datatype) | Convergence + algebraic laws | Varies; some CRDTs expensive | Specialized merge functions | Counters, sets, collaborative text (with caveats) |
...
Formal models also differ in **testability**. Linearizability has established checkers (Knossos-style histories). Eventual consistency without specified bounds is **hard to falsify** in short tests—bugs may manifest only after hours of replication drift.
...
### Storage-layer read models (often overlooked)

Beyond replication semantics, storage systems expose **per-read consistency knobs**:

- **Strong / linearizable reads** contact a quorum or leader
- **Bounded staleness** reads accept lag up to N seconds or versions
- **Consistent prefix** reads never see future writes without past writes
- **Eventual** reads may return any prior committed value

Misalignment between application assumptions and default read APIs causes more production bugs than misunderstanding CAP. A "strongly consistent database" with **async read replicas** is only strong on the primary path.
...
### Gray failures and partial partitions

Not all partitions are clean splits. **Slow nodes**, **degraded NICs**, and **asymmetric routing** produce intermittent timeouts. Systems configured to **fail open** on timeout may serve stale reads while believing they are healthy. **Correlated failures** (shared rack, regional power event) defeat naive multi-AZ assumptions.
