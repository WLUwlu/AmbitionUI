...
The **BASE** philosophy (Basically Available, Soft state, Eventual consistency) articulated the alternative to ACID for web-scale systems. Google's **Bigtable** (2006), **GFS**, and later **Spanner** (2012) showed different points on the spectrum: Bigtable offered per-row atomicity but not cross-row transactions initially; Spanner invested heavily in TrueTime (GPS/atomic-clock-assisted timestamps) to provide **external consistency** (linearizable transactions across shards) at global scale.

### 2010s–2020s: Refinement, CRDTs, and Formal Verification
...
**Mechanisms:** Two-phase locking (2PL), optimistic concurrency control (OCC), timestamp ordering, Percolator-style distributed transactions over a key-value layer.

**Representative systems:** PostgreSQL (SSI mode), CockroachDB, TiDB, FoundationDB, Spanner.

### 3.4 Causal Consistency

**Guarantee:** If operation A causally precedes B (A happened-before B), every process observes A before B. Concurrent operations may be observed in different orders.

**Mechanisms:** Vector clocks attached to writes; servers buffer and deliver updates respecting causal dependencies; session tokens passed between client and server.

**Use when:** Social feeds, messaging, collaborative tools where related events must appear ordered but unrelated events can diverge temporarily.

### 3.5 Session Guaranties

These are client-centric guarantees often bundled in practice:

- **Read-your-writes (RYW):** A client always sees its own prior writes.
- **Monotonic reads:** If a client reads value v, later reads never return older values.
- **Monotonic writes:** A client's writes are applied in order.
- **Writes-follow-reads:** If a client reads A then writes B, B is visible after A was read.

**Mechanisms:** Sticky routing to a primary or session-aware replica; version vectors per session; client-side read timestamps.

**Use when:** Web applications with logged-in users where personal experience matters more than global total order.

### 3.6 Eventual Consistency

**Guarantee:** If updates stop, all replicas eventually converge to the same value. No bound on convergence time is promised.

**Mechanisms:** Asynchronous replication; anti-entropy (Merkle trees, gossip); read repair; last-writer-wins (LWW) conflict resolution.

**Representative systems:** Dynamo-family (Cassandra, Riak, Voldemort), DNS, many CDN cache invalidation schemes.

**Use when:** Shopping carts, social likes, metrics aggregation, any domain where temporary divergence is acceptable and conflicts are rare or mergeable.

### 3.7 CRDT-Based Strong Eventual Consistency

**Guarantee:** Replicas converge without coordination; all replicas that have received the same set of updates hold identical state (strong eventual consistency).

**Mechanisms:** Mathematically defined merge functions (G-sets, OR-sets, counters, registers) that are commutative, associative, and idempotent.

**Use when:** Offline-first apps, collaborative editing (with appropriate CRDT types), geo-distributed counters and presence indicators.

---

## Section 4: Trade-offs Analysis

Choosing a consistency model is an engineering optimization under constraints, not a moral judgment. The major trade-off axes interact in ways that simple CAP diagrams obscure.

### 4.1 Latency vs Consistency

Stronger guarantees require coordination. Linearizable writes across regions may incur round-trip times measured in tens to hundreds of milliseconds. Quorum reads at W=ALL eliminate stale reads but multiply write latency. **Monotonic reads with sticky sessions** are a common compromise: route a user to a replica that is "fresh enough" via session routing, giving users perceived correctness without paying full linearization cost on every read.

### 4.2 Availability vs Consistency Under Partition

During a partition, CP systems (etcd, ZooKeeper, traditional sync-replicated SQL primaries) may reject writes or reads to avoid stale responses. AP systems (Dynamo-style, Cassandra with consistency level ONE) remain writable on both sides of a partition, accepting divergence and relying on conflict resolution later—the **split-brain** risk.

Dynamo's **sloppy quorum** and **hinted handoff** improve availability but complicate consistency analysis. The R+W > N quorum intersection rule (N replicas, R read, W write) guarantees overlap but only if the same N nodes are reachable—hinted handoff breaks naive quorum math.
...
- **Dirty read:** Read uncommitted data (prevented by Read Committed and above).
- **Non-repeatable read:** Same row differs within transaction.
- **Phantom read:** New rows appear in range scan.
- **Write skew:** Two transactions read overlapping sets and write disjoint rows, violating an invariant (classic example: on-call schedule requiring at least one doctor always on call).
