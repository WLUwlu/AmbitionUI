...
- **Geo-distributed SQL** (Spanner, CockroachDB, YugabyteDB, TiDB, Vitess/PlanetScale patterns)
...
**Bounded staleness.** A middle ground increasingly common in production: reads may lag writes by at most T seconds or K versions. Google Spanner and Azure Cosmos DB expose tunable consistency levels including bounded staleness. This model acknowledges that **users often tolerate small, quantified lag** if the system documents it.
...
### Trade-off axis 5: Cost and infrastructure economics

Strong consistency across regions implies **synchronous or semi-synchronous replication**, which increases write amplification, cross-AZ egress charges, and consensus round-trips. Eventual models allow **cheaper passive replicas** and edge caching. The "correct" model is not purely technical—finance teams may prefer paying for regional strong consistency on ledger paths while accepting eventual consistency on recommendation features.
...
Not all failures are clean crashes. **Slow replicas** that respond after timeouts can violate leader assumptions; **partial network partitions** let some clients see divergent states while health checks pass. These scenarios rarely appear in textbook CAP diagrams but dominate production incident timelines.
