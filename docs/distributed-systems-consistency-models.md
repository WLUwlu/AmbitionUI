...
### Isolation versus replication consistency

A recurring confusion: **transaction isolation** governs concurrent transactions on one logical database; **replication consistency** governs what replicas expose relative to one another. A system can be serializable on a primary yet serve stale reads from async replicas — violating read-your-writes without violating serializable isolation on the writer path. Production incidents often trace to conflating these layers.
