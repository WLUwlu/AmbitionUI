...
|-----------|------------|
| **Read-your-wwrites (RYW)** | A client always sees its own prior writes. |
...
The **FLP impossibility result** (Fischer, Lynch, Paterson, 1985) proves that in an asynchronous system with even one failure, no deterministic consensus protocol guarantees termination. This is why practical systems use partial synchrony assumptions (Raft's election timeouts), failure detectors, or randomized algorithms.
