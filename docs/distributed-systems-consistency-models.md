...
**Transactional isolation (serializability and weaker levels).** Multi-object guarantees: serializable execution means transactions appear to run one at a time. Snapshot isolation provides point-in-time reads but permits write skew. These are consistency models over transaction histories, not single-key reads alone.
