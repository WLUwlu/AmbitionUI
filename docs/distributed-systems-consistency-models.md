...
Transactional consistency addresses multi-key atomicity: transactions appear equivalent to some serial execution, or at minimum satisfy a chosen isolation level. Isolation and replication consistency are orthogonal axes. A system can offer serializable transactions on a single shard while cross-shard operations remain only eventually consistent, a combination that confuses teams who treat "ACID" as a monolithic guarantee.

Consensus, leader election, and membership change are the mechanical substrate beneath many strong models. Raft and Multi-Paxos provide ordered logs; etcd, Consul, and ZooKeeper expose coordination primitives; Spanner and CockroachDB embed consensus inside SQL engines. Understanding consistency requires understanding what happens when leaders fail, when quorums cannot be assembled, and when membership changes overlap with in-flight operations.

Finally, consistency is always scoped. Per-key linearizability differs from global linearizability across all keys. Per-session read-your-writes differs from global read-your-writes. Per-region strong consistency differs from cross-region strong consistency. Vendor documentation often uses the word "strong" without stating scope, and that omission is where most production surprises originate.
