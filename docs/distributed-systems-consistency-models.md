...
Distributed consistency models did not arrive as a polished taxonomy. They accumulated over decades as engineers repeatedly discovered that replication across machines, regions, and time breaks the illusion of a single shared memory. The intellectual lineage runs from multiprocessor memory ordering through database isolation theory, through the CAP-era reframing around partition tolerance, into today's geo-distributed SQL engines, offline-first clients, and conflict-free replicated data types. Each named guarantee is, in practice, a formalization of a failure that once burned a production system.

### Prehistory: ordering before replication (1960s–1970s)

Before "distributed database" was a product category, the problem appeared in shared-memory multiprocessors and early network operating systems. Programmers assumed that memory reads and writes composed into a sensible sequential story. Multiple processors observing the same address space fractured that story. Edsger Dijkstra's work on mutual exclusion and Leslie Lamport's formalization of the happens-before relation (1978) gave the field a vocabulary for reasoning about order without a global physical clock. Logical clocks were not yet "consistency models" in the database sense, but they established a durable insight: correctness in distributed settings is about legal observable orderings, not about simultaneous truth at every node.
...
**Transactional consistency.** Multi-key atomicity with a serial order equivalent to some sequential execution. Serializable isolation is the gold standard for OLTP invariants. Snapshot isolation offers a weaker but often sufficient view with fewer coordination costs, at the price of anomalies like write skew unless augmented with additional detection.

**CRDT convergence.** Conflict-free replicated data types guarantee that replicas merge into a common state without coordination, given defined operations and merge functions. Convergence is algebraic, not temporal: the result may not match any single user's intent.

### The happens-before relation as glue

Lamport's happens-before relation connects these models. Linearizability respects real-time happens-before. Causal consistency respects message-passing and read-write happens-before. Eventual consistency guarantees convergence when the happens-before graph stops growing. Many production bugs trace to code paths that drop causal metadata—async queues, batch jobs, admin scripts—thereby violating guarantees the storage layer still claims to provide.

### Consistency is scoped, not global

Modern systems rarely offer one guarantee everywhere. A single product may combine linearizable leader election, serializable transactions within a region, eventual cross-region replication, and CRDT-based offline edits on mobile clients. The engineering task is to make scopes explicit: which API, which object class, which session, which failure mode. Ambiguity here is more dangerous than choosing a weaker model deliberately.
