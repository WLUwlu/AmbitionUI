...
Minicomputers and client-server architectures increased rack densities. The **raised-floor plenum** became a duct system: cold air supplied underfloor, drawn through perforated tiles into hot aisles, returned warm to CRAC intakes. Thermodynamically, this is a **forced-convection recirculation loop** with poorly characterized flow fields. The social and engineering innovation was treating **airflow path** as infrastructure as critical as power distribution.
...
Modular datacenters and factory-built cooling plants treated thermal systems as **repeatable thermodynamic appliances** rather than one-off building engineering. Meanwhile, GPU and HPC clusters exposed **spatial non-uniformity**: thermal load per rack became heterogeneous, breaking assumptions of uniform floor-tile airflow allocation.

### AI era, liquid cooling, and chip-level heat flux (2020s–present)
...
The thermodynamic narrative has inverted again: the bottleneck moved from room airflow to **junction-to-coolant conductance** and **facility water inventory**. Chip vendors now specify liquid cooling as a requirement, not an option, for frontier accelerators. This represents the most significant shift in datacenter thermodynamics since the raised-floor plenum—because it changes the **primary working fluid** from air to liquid or vapor at the point of highest heat flux.

---

## Section III — Thermodynamic Structure and Mechanisms

### Heat generation hierarchy and timescales

Heat in datacenters originates at the **silicon junction**, where electrical resistance and switching losses convert ordered electrical energy into disordered thermal energy. The conversion is effectively instantaneous at electronic timescales (nanoseconds), but thermal propagation occurs across multiple time constants:

- **Microseconds to milliseconds:** junction and die thermal mass; relevant for burst workloads and thermal throttling decisions
- **Seconds to minutes:** heat spreader, cold plate, and server chassis equilibration
- **Minutes to hours:** room air or facility water loop response; CRAC/chiller plant lag
- **Hours to days:** building fabric and ground-coupled thermal storage

At facility steady state, **IT electrical power equals heat rejection rate** to the environment (plus any net storage in thermal mass). This first-law closure is non-negotiable; disputes arise only over boundary definitions and measurement lag.

### The thermal resistance network

Heat flows from junction to ambient through a **series-parallel resistance network**:

1. **Junction to case** — semiconductor thermal resistance, package design
